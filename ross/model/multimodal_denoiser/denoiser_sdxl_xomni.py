...
class RossStableDiffusionXLXOmni(nn.Module):
    def __init__(
        self,
        unet_path,
        z_channel,
    ...
    def forward(self, z, target):
        ...
        pooled_encoder_hidden_states = torch.load(self.negative_pooled_prompt_path).to(target.device).to(target.dtype)
        pooled_encoder_hidden_states = pooled_encoder_hidden_states.repeat(bsz, 1)
        ...
        ).to(target.device).to(target.dtype)
