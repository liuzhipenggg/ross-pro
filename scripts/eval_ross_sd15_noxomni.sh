#!/usr/bin/env bash
# Eval-only for sd15 non-xomni SFT (already trained).
# Align with prior Ross flow: MMVP (8-way) → VLMEval (8-GPU) → reconstruct_allbench.
#
# Usage:
#   source activate_ross.sh
#   bash scripts/eval_ross_sd15_noxomni.sh
set -euo pipefail
...
# Drop nested duplicate save (wastes ~17G, confuses loaders)
if [[ -d "$CKPT/checkpoint-7676" ]]; then
  echo "Removing nested $CKPT/checkpoint-7676"
  rm -rf "$CKPT/checkpoint-7676"
fi
...
# MMVP/VLMEval already cached under HF_HOME; avoid hanging on hub network checks
...
echo "ckpt=$CKPT  nproc=$NPROC  LMUData=$LMUData"

# ---- MMVP 8-way (fp16 load; stagger to ease disk) ----
echo "======== $(date) MMVP ========"
cd "$ROOT/MMVP"
...
# ---- VLMEval: same sets as official reconstruct (covers score + allbench inputs) ----
echo "======== $(date) VLMEval ========"
cd "$ROOT/VLMEvalKit"
DATASETS=(
  POPE
