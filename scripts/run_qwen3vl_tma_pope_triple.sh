...
SCRIPT="$ROOT/scripts/eval_qwen3vl_tma_pope_triple.py"
OUT="$ROOT/outputs/qwen3vl_tma_pope_triple"
LOG="$ROOT/logs/qwen3vl_tma_pope_triple.log"
NPROC="${NPROC:-8}"
...
echo "======== $(date) Qwen3-VL TMA+POPE triple START nproc=$NPROC ========"
...
"$PY" "$SCRIPT" --merge "$OUT"
echo "======== $(date) Qwen3-VL TMA+POPE triple DONE ========"
df -h /mnt/vdb1 | tail -1
